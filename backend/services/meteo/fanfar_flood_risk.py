...
# Assurez-vous d'importer les classes et variables nécessaires
# Ceci est une simulation des imports requis (config, logger, FloodRiskResult)
try:
    from playwright.sync_api import sync_playwright, Route, Response
    # Les classes et variables suivantes (config, logger, FloodRiskResult) 
    # sont supposées exister dans l'environnement d'exécution.
    # Pour que ce code soit exécutable tel quel, ces dépendances devraient être définies.
    class Config:
        HEADLESS_MODE = True
    ...
    # Définir des classes mock pour éviter l'échec si l'utilisateur veut juste voir le code
    class Route: pass
    class Response: pass
    def sync_playwright(): return None
    class Config: pass
    config = Config()
    class Logger: pass
    logger = Logger()
    class FloodRiskResult:
        def __init__(self, location, risk_level, risk_score, source_type, details, station_name=None, screenshot_path=None): pass
