...
def __getattr__(name: str):
    ...
    if name == 'DocumentScraper':
        from backend.services.data_collection.weather.documents_meteo import DocumentScraper
        return DocumentScraper

    if name == 'WeatherForecastService':
        from backend.services.data_collection.weather.weather_forecast import WeatherForecastService
        return WeatherForecastService

    if name == 'SonagessScraper':
        from backend.services.data_collection.documents.sonagess import SonagessScraper
        return SonagessScraper

    if name == 'AnamBulletinScraper':
        from backend.services.data_collection.documents.fews_pdf_harvester import AnamBulletinScraper
